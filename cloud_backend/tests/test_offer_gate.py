...
def _publish(client, offer):
    ...
    r = client.post("/founder/map-state",
                    headers=_auth("founder.desktop@example.test"), json=payload)
...
class TestMapPushTimestamp:
    ...
    def test_a_fresh_map_is_served_live(self, client):
        ...
        asset = client.get("/founder/map-assets/map-data.js",
                           headers=_auth("founder.desktop@example.test")).text
...
class TestBothFounderAddressesOwnTheCockpit:
    ...
    @pytest.mark.parametrize("email", ["founder.desktop@example.test",
    ...
    def test_the_secret_is_the_only_source(self, monkeypatch):
        import config
        monkeypatch.setenv("FOUNDER_EMAIL", "solo@x.test")
        assert config.founder_emails() == frozenset({"solo@x.test"})
        monkeypatch.setenv("FOUNDER_EMAIL", " A@x.test , b@Y.test ")
        assert config.founder_emails() == frozenset({"a@x.test", "b@y.test"})
        monkeypatch.delenv("FOUNDER_EMAIL")
        assert config.founder_emails() == frozenset()   # no default: fail closed

    def test_purge_protection_covers_both_addresses(self):
        import db
        protected = db._protected_emails()
        assert "founder.desktop@example.test" in protected
        assert "founder@example.test" in protected
