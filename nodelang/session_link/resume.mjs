...
export function requiredEndpoints(binding, all) {
 const app=binding.claude.app||'claude';
 ...
 return remote.length===1&&codex.length===1?{app,remote:remote[0],codex:codex[0]}:null;
}
export async function resumeSaved(binding, {probe, discover, connect, currentExecutor}) {
 ...
 if(!['live','offline'].includes(held.status))return {...held,dispatch_attempted:false};
 if(held.status==='offline' && binding.permissionMode==='bypass' && (!currentExecutor || binding.executor!==currentExecutor))
   return {status:'recovery_required',reason:'saved_sender_permission_origin_changed',dispatch_attempted:false};
 const endpoints=requiredEndpoints(binding,await discover());
 if(!endpoints)
   return {status:'recovery_required',reason:'saved_endpoint_not_exactly_one_live_session',dispatch_attempted:false};
 if(held.status==='live')return {...held,status:'already_connected',endpoints_observed:true,dispatch_attempted:false};
 const restored=await connect({app:endpoints.app,claude:endpoints.remote.selector||endpoints.remote.id,
   codex:endpoints.codex.id,permissionMode:binding.permissionMode||'prompting'});
 return {status:'restored',connection:restored,endpoints_observed:true,dispatch_attempted:false,work_recovery_required:true};
}
