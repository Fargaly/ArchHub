...
def _normalized_snapshot_digest(
    ...
    return snapshot_digest(Snapshot(snapshot.revision, MappingProxyType(cells)))
...
def revise_relation_node(
    ...
    *,
    ...
    """Revise one existing relation node in place, under its own root.

    A caller states the members it wants the relation to end with. A member
    carrying an incidence_id keeps that incidence, so an edge that survives a
    revision keeps its identity instead of being deleted and recreated under a
    new one; a member without an incidence_id is new and gets a fresh
    incidence; an existing member the caller omits is removed. The relation
    root does not change, so anything pointing at this relation keeps pointing
    at it.

    Order is taken from the caller rather than sorted here: the sequence a
    reader sees is the sequence the command asked for.
    """
    if type(relation_root) is not str or not relation_root:
        raise InvalidCell("relation root is invalid")
    if type(expected_revision) is not int:
        raise InvalidCell("relation revision base is invalid")
    normalized: list[tuple[str | None, str, str]] = []
    for member in members:
        if not isinstance(member, Mapping):
            raise InvalidCell("relation member is invalid")
        role_root = member.get("role_root")
        participant_root = member.get("participant_root")
        incidence_id = member.get("incidence_id")
        if type(role_root) is not str or not role_root:
            raise InvalidCell("relation member role is invalid")
        if type(participant_root) is not str or not participant_root:
            raise InvalidCell("relation member participant is invalid")
        if incidence_id is not None and type(incidence_id) is not str:
            raise InvalidCell("relation member incidence is invalid")
        normalized.append((incidence_id, role_root, participant_root))
    if len(normalized) < 2:
        raise InvalidCell("a revised relation requires at least two members")
    request_digest = _digest({
    ...
    current = {
        member.incidence_id: member
        for member in read_relation(snapshot, relation_root, budget=100_000)
    }
    declared_roles = set(authority.roles.values())
    kept: set[str] = set()
    create: list[Cell] = []
    replace: list[Cell] = []
    for incidence_id, role_root, participant_root in normalized:
        if role_root not in declared_roles:
            raise InvalidCell("relation member role is not declared")
        if participant_root not in snapshot.cells:
            raise InvalidCell("relation member participant is missing")
        if incidence_id is None:
            create.append(Cell(_new_id(), role_root, participant_root, b""))
            continue
        if incidence_id not in current:
            raise InvalidCell("relation member incidence is not in this relation")
        kept.add(incidence_id)
        held = current[incidence_id]
        if held.role_id != role_root or held.participant_id != participant_root:
            replace.append(Cell(incidence_id, role_root, participant_root, b""))
    # An omitted member is a removal. The incidence is retired by pointing it
    # at the null cell, which keeps the identity readable as having existed
    # rather than erasing it from the record.
    for incidence_id in current:
        if incidence_id in kept:
            continue
        replace.append(Cell(incidence_id, NULL_CELL_ID, NULL_CELL_ID, b""))
    return _commit_with_receipt(
        authority,
        snapshot,
