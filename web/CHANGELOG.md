...
The "declutter the rail and the composer" release. With the router,
connectors, brain, and account chip all landed and honest (v1.6.1–v1.6.6),
the remaining noise was visual: too many always-on rail items and composer
controls competing for the eye. v1.6.7 strips the studio rail and composer
back to what a user actually clicks within 60 seconds, and surfaces the
rest behind disclosure.

### Changed

- **Rail declutter** — the studio left rail shows the primary surfaces
  (Canvas / Sessions / Library / Brain) by default; secondary entries fold
  behind a "More" disclosure instead of crowding the steady-state view.
- **Composer declutter** — the composer keeps the controls a turn actually
  needs (mode, attach, send) in the resting state; advanced affordances stay
  one click away rather than always-on. Multi-line input and the first-message
  path (fixed in v1.6.5) are unchanged.

### Notes

- No behaviour change to the router, connectors, or brain — this is a
  surface-noise pass on top of the v1.6.x reliability work.

## [1.6.6] — 2026-06-20

The "honest connector status" release. A founder ran "ping notion" and got
"notion is not reachable (unknown)" — concluding the router was broken, when
the real cause was simply no Notion integration token configured.

### Fixed

- **Unconfigured token-hosts now say "add your token", not "not reachable
  (unknown)"** — fixed at the mechanism for the whole token-REST connector
  class (Notion / Dropbox / Teams / Procore / Speckle):
  - `host_detector.probe_notion` was a wrong-mechanism desktop-process probe
    (false-"live" on any process named "notion", token ignored). It now defers
    to the REST connector's token-aware `probe()`.
  - `bridge.probe_connector` cached a status-less `{error}` dict on the raise
    path, so the UI rendered "(unknown)". A new normaliser guarantees a named
    status: a token-REST host with no/invalid token → `unauthorized` plus
    "add your token in Settings → Sign-ins → X".
  - The router's tool-result summariser now says "X not connected — add your
    integration token…" instead of a misleading "reachable=False".
- Three honest states kept distinct: token missing / token rejected (401) /
  host genuinely unreachable.

## [1.6.5] — 2026-06-20

The "composer reliability" release. Behind the founder's "every time I write
in the composer I get nothing back" were two live-diagnosed bugs plus a class
of router turns that could hang or come back blank.

### Fixed

- **Home composer no longer drops your first message.** Submitting from Home
  created the session then fired a `window` event that RACED the freshly
  mounting session view and was lost (the send never fired). And any message
  containing a host-family substring (word / max / teams / notion / excel /
  revit…) without a verb fell into a bare branch that created a session with
  NO message. The first message now threads through session creation
  deterministically — no window event, no race.
- **In-session composer multi-line** restored alongside the first-message fix.
- **No more blank or hung turns** — the router got a per-turn watchdog / turn
  budget and connector-call timeouts so a slow or stuck provider/connector
  can't leave a turn spinning forever or return an empty bubble.

## [1.6.4] — 2026-06-19

A re-landing release: two UI fixes committed to the v1.6.3 branch missed its
auto-merge and had shipped only as a hot-patch. v1.6.4 lands them cleanly on
main so auto-update carries them.

### Fixed

- **Stem-node Properties clarity** — the Properties panel now teaches the
  node model ("PROPERTIES — this node's settings, its definition" and, when a
  value is promotable, "Tap ⊙ to turn a value into a wired input socket").
- **De-duplicated masthead** — one brain chip (was two); the router status
  renders only the live model Auto resolved to (no twin "Auto (router picks)").

## [1.6.3] — 2026-06-19

The "kill the 80%-done look" release. The footer hand-typed "v1.4 prototype"
on every screen regardless of the running version, and a node-output panel
read "3D VIEWER — COMING SOON" — both made a shipped app look like a prototype.

### Fixed

- **Footer shows the REAL version.** The footer now renders the live version
  from `get_version()` (the single source of truth, reading the VERSION file
  the release writes), gated on readiness and retrying until it lands —
  no more frozen "v1.4 prototype" literal, no scattered alpha fallbacks.
- **Performance auto-recovery** — the app self-recovers a degraded render
  path instead of leaving a slow/blank surface (extends the v1.5.0 GPU
  never-blank work).
- **Honest geometry inspector** — the view/model node output shows real
  vertex / face / item counts instead of a "coming soon" placeholder.

## [1.6.2] — 2026-06-18

### Fixed

- **Last UI shells closed** — flagged dead surfaces were either made real or
  honestly removed; Settings controls are now all wired or gone.
- **Logical session graph** — opening a chat renders it as a clean,
  collapsible node graph (a proper DAG layout) instead of one flat node.

## [1.6.1] — 2026-06-18

### Added

- **AccountChip on the Home header** — your cloud/account state (plan,
  sign-in, remaining credits) is now visible on the landing surface, driven
  by the real `cloud_status` — no placeholder "Fargaly / BYO" literals.
...
The "make it visible" release. The cloud, brain, router, and sessions all
worked but weren't surfaced; v1.6.0 brings them onto the running UI and
consolidates the design language into one source of truth.

### Added

- **Self-Heal Inspector** — self-healing connectors are now VISIBLE as a live
  recovery timeline (watch → diagnose → action → restore), so the user can see
  the reconnect happen instead of just trusting it.
- **Live backends visible in the UI** — cloud, brain, and the router's resolved
  model are surfaced in the top nav / masthead.
- **Cross-device session sync** — node-graph sessions sync via the cloud with
  large-blob caps.
- **Team / Workspace permissions tab** — a native UI mirroring the live cloud
  roles model.
- **Design system as the literal source of truth** — tokens live in
  `window.AH` (the terracotta accent, the 12-step scale, the wire vocabulary),
  consumed by both the app and this website so they can't drift.
- **Voice lint (§11)** — advisory CI that enforces the studio language rules
  (no emoji / hype / exclamation) on UI copy.

## [1.5.0] — 2026-06-15

The "never blank, always fast" finalization release — plus a 32-commit
backlog of brain-federation, connector, and CI work.

### Fixed

- **GPU never-blank surface** — a software-render self-heal replaces the old
  environment-variable band-aid, so the embedded UI can't paint blank on a
  flaky GPU context.
- **Fast canvas** — the node/wire render is viewport-culled so large graphs
  stay responsive; cloud / brain / router state surfaced in the top nav.
- **Sessions as graphs** — every chat opens as a modular node graph, not one
  flat node.
- **Boot-hang root cause** — provider probes (LM Studio, etc.) that ran on the
  Qt main thread at startup are now off-thread, so the app can't hang on boot.
- **AutoCAD host-name unified** (acad / autocad) across the app.

### Added

- Brain federation hardening — server-side sandboxed re-execution + signed
  attestation for skill verification, a meaning-space recall ranker, persisted
  federation outbox + reputations, and an export-time legal/training-rights gate.
- Speckle send/receive now uploads the full object tree and dereferences
  children on receive.
- Real agents CI deploy + smoke; brain + agents test suites wired into CI.

## [1.4.2] — 2026-06-12

Ships this session's court-verified PRs.

### Fixed

- Boot-hang root cause, brain-driver wiring, connector geometry + write-gating,
  panel fixes, a docs-freshness gate, the two-store brain unify, and the
  cloud Dockerfile (Node 22 base).

## [1.4.1] — 2026-06-10

### Fixed

- **Accessibility font scale no longer clips the UI** off the window edge.

## [1.4.0] — 2026-06-09

The first release since v1.3.3 — the production update channel plus a large
backlog (75 PRs).

### Added

- **Safe, forward-only, user-controlled updates** — dev-source updates no
  longer self-apply or resurface old data; production updates ship as signed
  GitHub Releases with an in-app banner.
- **Param → socket promote** — toggle a connector knob into a typed input
  socket directly on the canvas.
- **Intent-ranked discovery palette** — the node library ranks by intent and
  shows visible plugs.
- **Persistent MCP server** — the chat→host path starts a persistent MCP
  server on launch, ending the per-turn startup race that made the first tool
  call fail.
- **Stem-cell node library** — the 13 messy composite nodes rebuilt in place as
  modular stem-cell compositions; the param-edit re-cook now actually changes
  the output.

### Fixed

- Rhino bridge forces the CPython-3 engine and arms on load.
- Connector deploy gate (net48 + per-target-framework sha pins + manifest
  ownership guard).
