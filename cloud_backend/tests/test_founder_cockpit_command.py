...
class TestDirectAgentRealQueue:
    ...
    def test_build_queues_the_task_and_dials_no_retired_brain(self, client, monkeypatch):
        """The personal Brain daemon is retired: no BRAIN_MCP_URL, no local port."""
        import urllib.request
        from pathlib import Path

        def refuse(*_a, **_k):
            raise AssertionError("the build command dialed the network")
        monkeypatch.setattr(urllib.request, "urlopen", refuse)
        ftoken = _sign_in(client, monkeypatch, FOUNDER_EMAIL)
        d = client.post("/founder/api/command", headers=_auth(ftoken),
                        json={"command": "build a Notion connector for me"}).json()
        assert d["ok"] is True and d["queued"] is True and "brain" not in d
        source = (Path(__file__).resolve().parents[1] / "founder_cockpit.py").read_text(encoding="utf-8")
        assert "BRAIN_" + "MCP_URL" not in source and "84" + "73" not in source

    def test_the_system_panel_says_the_replica_sync_is_stopped(self, client, monkeypatch):
        import founder_cockpit
        replicas = founder_cockpit._system_panel()["brain_replicas"]
        assert replicas["sync"].startswith("stopped: the local Brain daemon")
