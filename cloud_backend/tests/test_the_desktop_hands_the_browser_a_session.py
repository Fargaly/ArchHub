...
# --- One sign-in ----------------------------------------------------------------
# The cockpit had three sign-ins of its own (token form, emailed link, Google
# button) beside the app's. Founder, 2026-09-24: "how many places do we sign in
# to the cloud and the cockpit?" The app is the one door; this link is its hand-off.


def test_the_door_page_sends_the_founder_to_the_app(client):
    page = client.get("/founder/login").text
    assert "<form" not in page and "Continue with Google" not in page
    assert "Open the cockpit" in page


def test_a_cockpit_state_on_the_google_return_mints_no_cockpit_session(client):
    """The cockpit's own Google finisher is gone with its button."""
    import db
    ...
    assert "founder_session" not in landed.headers.get("set-cookie", "")


def test_the_claim_cookie_is_server_minted_and_hardened(client):
    token = _token_for(FOUNDER_EMAIL)
    claim = client.post(
        "/founder/api/browser-code",
        headers={"Authorization": "Bearer " + token},
    ).json()["claim_url"]
    code = claim.split("code=", 1)[1]
    landed = client.get("/founder/claim?code=" + code, follow_redirects=False)
    cookie = landed.cookies.get("founder_session")
    assert cookie and cookie != code and cookie != token
    set_cookie = landed.headers.get("set-cookie", "")
    assert "HttpOnly" in set_cookie and "Secure" in set_cookie and "Path=/founder" in set_cookie


def test_a_hostile_state_never_steers_a_redirect(client):
    hostile = client.get(
