...
function LiveDomainControl(_ref2) {
  ...
  var allItems = ctl ? ctl.work_items || [] : [];
  var words = [low, String(d.key || '').toLowerCase()].concat(_toConsumableArray(low.split(/[^a-z0-9]+/).filter(function (w) {
    return w.length > 3;
  })));
  var scoped = allItems.filter(function (w) {
    var t = String(w.title || '').toLowerCase();
    return words.some(function (x) {
      return x && t.includes(x);
    });
  });
  var items = scoped.length ? scoped : allItems;
  var itemsLabel = scoped.length ? 'GOVERNED WORK · THIS DOMAIN' : 'GOVERNED WORK · ALL';
  ...
  }, itemsLabel), ctl.work_summary && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: HB.ink,
      marginBottom: 4
