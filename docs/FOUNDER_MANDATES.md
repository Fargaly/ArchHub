...
Carried from the retired `10.PRODUCT/12.PRODUCTION` (`CLAUDE.md` and `AGENTS.md`
at `e17454f`) so that this repository holds its own rules (WORKSPACE-STANDARD:
"preserve and reconcile valid founder mandates"). The mandate text below is
verbatim. Not carried: 12 `CLAUDE.md` "What ArchHub is", "Commands", "Hard-won
root causes" and "Key files" (facts about the v1 Qt application, not rules) and
12 `AGENTS.md` section 3 (a mirror of the mandates below). All of it stays
readable as `git show e17454f:CLAUDE.md` and `git show e17454f:AGENTS.md`.

## Reconciliation with the current authority (recorded, not silently edited)

The current founder decisions, `00.GOVERNANCE/WORKSPACE-STANDARD.md` and
`AUTHORITY.md` control where a mandate below conflicts with them:

- **BRAIN-FIRST** names the `personal-brain-mcp` daemon on
  `http://127.0.0.1:8473/mcp`, `brainwrap` and `personal-brain` installers. That
  daemon is retired: the application's own Brain replaces it (founder decision
  2026-09-15/16; WORKSPACE-STANDARD :490 "one primary database/Brain/Workshop
  authority per instance"; `AUTHORITY.md` "Brain belong[s] to this
  application"). The mandate's intent (use shared memory before and after work)
  stands; its daemon, port and launcher do not.
- Paths such as `studio-lm.jsx`, `payload/sources/**/*.cs`, `docs/agdr/`,
  `docs/ROADMAP.md` and `tools/brainwrap` name the retired v1 checkout. The
  rules attached to them (AgDR before contract changes, one roadmap, prototype is
  contract) apply to this repository's equivalents; the paths are historical.
- Where a mandate and `SPEC.md` / `AUTHORITY.md` disagree on architecture, the
  specification controls (`CLAUDE.md` of this repository).
...
# Cross-vendor rules (12 AGENTS.md sections 1-2, verbatim)
