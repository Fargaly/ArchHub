...
class LLMRouter:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "over per-item loops:\n"
            "   • Categorise many emails: outlook_set_categories_"
            "by_filter(categories=[...], sender_contains=..., "
            "subject_contains=..., days=...) — ONE call, no loop.\n"
            "   • Group inbox by sender: outlook_list_distinct_"
            "senders() — returns domains + counts + sample subjects "
            "so you can propose project names WITHOUT reading every "
            "message body.\n"
            "   Typical 'categorise all my emails by project':\n"
            "     1. outlook_list_distinct_senders() → see domains\n"
            "     2. for each domain: outlook_set_categories_by_"
            "filter(sender_contains=domain, categories=['<name>'])\n"
            "   ONLY fall back to a per-item loop "
            "(list_inbox → read_thread → set_categories per item) "
            "when the bulk tool doesn't fit. In that case use the "
            "REAL entry_id from item['entry_id'] — never a "
            "placeholder string like '[each message]'.\n"
