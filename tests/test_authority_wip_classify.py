...
def test_classification_keeps_universal_cell_separate_from_legacy():
    ...
    assert (
        awc.classify_path("docs/_meta/legacy_runtime_handoff_stale_stdin_cleanup.latest.json")
        == "governance_run_evidence"
    )
    assert (
...
def test_public_wip_maintenance_runbook_exists_and_names_the_required_gates():
    ...
    required_phrases = [
        ...
        "personal-brain-mcp\\tests\\test_hook_coverage.py",
        "tests\\test_antigravity_governance_hooks.py",
...
def test_external_worktree_leaf_gate_executes_owner_freshness_court():
    report = awc.classify_entries([
        {
            "code": " M",
            "path": "external-worktree:analyze-logs-performance/app/bridge.py",
    ...
    leaf = report["active_work_leaves"][0]
    expected_courts = sorted(awc.EXTERNAL_WORKTREE_COURTS)

    assert leaf["governance_context"]["category"] == "external_owner_worktree_wip"
    assert leaf["gate_spec"]["selectors"] == [
        "tests/test_authority_wip_classify.py",
        *expected_courts,
    ]
