...
JSX side doesn't have to re-implement graph surgery in JS:
  ...
  * ``ArchHubBridge.save_as_skill(name, payload_json)`` — packages a
    ...
    into the writable skill store
    ``%LOCALAPPDATA%/ArchHub/skills/<slug>.archhub-skill.json``.
...
method. Pins:
  ...
  - save_as_skill writes a file with the right shape to the user store
...
# ══════════════════════════════════════════════════════════════════════
# 1. save_as_skill — writes the JSON to the writable user skill store
# ══════════════════════════════════════════════════════════════════════
class TestSaveAsSkill:
    """save_as_skill writes a canvas skill to the writable user store
    (%LOCALAPPDATA%/ArchHub/skills/ — the same store get_saved_skills +
    load_skill read). The conftest autouse fixture redirects
    LOCALAPPDATA to a per-test tmp dir, so these never touch the real
    store; no __file__ monkeypatch needed."""

    def test_writes_file_with_envelope_shape(self, bridge_inst, small_graph):
        payload = json.dumps(small_graph)
        result = json.loads(
            bridge_inst.save_as_skill("My Tower Skill", payload))

        assert result.get("ok") is True
        assert "error" not in result
        out_path = Path(result["path"])
        assert out_path.exists()
        # Written into the writable user store, never the source tree.
        assert out_path.parent == _bridge_module._user_skills_dir()
    ...
    def test_slugifies_messy_names(self, bridge_inst, small_graph):
        # Punctuation, casing, leading/trailing junk.
        result = json.loads(bridge_inst.save_as_skill(
    ...
    def test_empty_name_falls_back_to_untitled(self, bridge_inst, small_graph):
        result = json.loads(
            bridge_inst.save_as_skill("", json.dumps(small_graph)))
        assert result.get("ok") is True
    ...
    def test_creates_skills_dir_if_missing(self, bridge_inst, small_graph):
        # The user store may not exist yet — save_as_skill (via
        # _user_skills_dir) must create it rather than erroring.
        result = json.loads(
            bridge_inst.save_as_skill("fresh", json.dumps(small_graph)))
        assert result.get("ok") is True
        skills_dir = _bridge_module._user_skills_dir()
        assert skills_dir.is_dir()
        assert Path(result["path"]).parent == skills_dir
        assert Path(result["path"]).exists()

    def test_bad_json_returns_error(self, bridge_inst):
        result = json.loads(
