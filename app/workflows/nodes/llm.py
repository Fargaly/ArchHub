...
def _build_history(prompt: str, system: str | None) -> list[dict]:
    history: list[dict] = []
    if system:
        # System prompts are passed via the per-provider client, not in `messages`.
        # Anthropic/OpenAI clients both expect the system prompt separately,
        # but llm_router.complete() composes it from active connectors. We
        # prepend a synthetic user/assistant exchange when an explicit system
        # is supplied at the node level.
        history.append({"role": "system_override", "content": system})
