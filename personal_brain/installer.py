"""Legacy client-wiring definitions retained for read-only hook observation.

Canonical ArchHub uses the constants, client detectors and configuration readers
from this module. Its legacy repair CLI is disabled: relative workspace command
generators are not an admitted installation path in the consolidated package.
Historical implementation context follows; it is not current run instructions.
...
if __name__ == "__main__":  # pragma: no cover
    raise SystemExit("Legacy Brain installer is disabled; use the ArchHub application installer.")
