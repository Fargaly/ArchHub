...
def _claim(client, token, **kw):
    """The one way in: the app spends its session for a one-time claim link."""
    claim = client.post("/founder/api/browser-code", headers=_auth(token)).json()["claim_url"]
    return client.get("/founder/claim?code=" + claim.split("code=", 1)[1], **kw)


class TestCookieLogin:
    """The browser path: GET /founder/login is an ungated door page with no
    sign-in of its own; the app's one-time claim link mints a
    `founder_session` cookie, and that cookie then authenticates /founder
    exactly like the bearer header does."""
    ...
    def test_login_page_is_ungated_200(self, client):
        ...
        assert "text/html" in r.headers["content-type"]
        html = r.text
        # On-brand, and no sign-in of its own: the cockpit opens from the app
        # (Settings, Account), on the one account signed in there.
        assert "#d97757" in html
        assert "Instrument Serif" in html and "Inter" in html
        assert "<form" not in html and 'name="token"' not in html
        assert 'name="email"' not in html and "/founder/login/google" not in html
        assert "Open the cockpit" in html

    def test_founder_claim_sets_cookie_and_redirects(self, client, monkeypatch):
        # (b) The app's claim link for the FOUNDER -> 303 + Set-Cookie founder_session.
        token = _sign_in(client, monkeypatch, FOUNDER_EMAIL)
        r = _claim(client, token, follow_redirects=False)
    ...
    def test_full_browser_flow_post_then_get(self, client, monkeypatch):
        ...
        # browser: claim link -> 303 -> GET /founder, then a fresh plain
    ...
    def test_the_cockpit_has_no_sign_in_of_its_own(self, client, monkeypatch):
        # The token form, the emailed cockpit link and the cockpit Google
        # button are gone: a posted founder token mints nothing.
        token = _sign_in(client, monkeypatch, FOUNDER_EMAIL)
        r = client.post("/founder/login", data={"token": token},
                        follow_redirects=False)
        assert r.status_code == 405, r.text
        assert "founder_session=" not in r.headers.get("set-cookie", "")
        for path in ("/founder/login/email", "/founder/login/google"):
            assert client.post(path, data={"email": FOUNDER_EMAIL},
                               follow_redirects=False).status_code in (404, 405)
            assert client.get(path, follow_redirects=False).status_code in (404, 405)
...
class TestGateInternals:
    def test_default_founder_email_constant(self):
        import founder_cockpit
        assert not hasattr(founder_cockpit, "DEFAULT_FOUNDER_EMAIL")  # no address in code
