...
export class PeerEndpoint {
  ...
  #receiveMessage(message) {
    ...
    const correlated = !gated && pending && (!pending.transcriptSession || (record.source === "transcript" && record.inReplyTo === key));
  ...
  waitForReply(fromSocket, { timeoutMs = 120000, since = Date.now(), afterSequence = null, msgId } = {}) {
    ...
    const matches = (record) => !["held", "rejected", "denied"].includes(this.deliveryReceipts.get(msgId)?.status)
  ...
  readDelivery(msgId) {
    ...
    if (!sent) return null;
    const delivery = this.deliveryReceipts.get(msgId);
    const gated = delivery && ["held", "rejected", "denied"].includes(delivery.status);
    ...
    return {
      ...
      ...(sent.reply && !gated ? { reply: sent.reply.text, source: sent.reply.source ?? "peer", ...(sent.reply.absorbed ? { replyAbsorbed: true } : {}), ...(sent.reply.forwardingError ? { forwardingError: { ...sent.reply.forwardingError } } : {}) } : {}),
    };
  }
