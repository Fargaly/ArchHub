...
C8. **PortType vs speckle_type drift.** AgDR-0012 §232-233 deprecates PortType; 33 source files still use it, 4 use speckle_type. Migration was never executed. Fork F3.
   > **Resolved 2026-05-26 per Q4 founder pick = rewrite (override of F3.C recommended)**: the migration shipped 4 stages as a DUAL-SYSTEM BRIDGE rather than the impossible literal "rewrite 33 files" interpretation. PortType remains the canvas-internal type system (25 values including EXEC/CRON/TRIGGER/EVENT control flow with no Speckle equivalent); speckle_type remains the wire-transport protocol. Stages: (1) bidirectional adapter `PortType.to_speckle_type` / `.from_speckle_type` with `archhub.*` namespace for non-Speckle + `Objects.*` for Speckle-native — commit `37ea0a7`; (2) Port.to_dict emits both keys, from_dict prefers new — `37ea0a7`; (3) Workflow.to_dict enriches edges with derived speckle_type — `e629a64`; (4) JSX wire renderer prefers SPECKLE_WIRE[w.speckle_type] with PortType fallback — `435854b`. AgDR-0012 §232-233 amended with the resolution. Stages 5/6 (node-spec declaration + PortType drop from coercion) deferred — the dual system already meets the original AgDR-0012 goal without a destructive rewrite.
