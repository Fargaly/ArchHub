...
class LLMRouter:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return (
            ...
            "them by project') are a LOOP. The pattern is ALWAYS:\n"
            "     a. Call the LIST/SEARCH tool first to get real IDs.\n"
            "     b. For each item in the returned list, use the ID "
            "        from that item — DO NOT make up IDs or use "
            "        placeholder strings like '[each message]'.\n"
            "     c. Loop the per-item tool until done.\n"
            "   Example: read_thread + set_categories on every inbox\n"
            "   item =>\n"
            "     1. outlook_list_inbox(limit=100)\n"
            "     2. for item in result['items']:\n"
            "        a. outlook_read_thread(entry_id=item['entry_id'])\n"
            "        b. outlook_set_categories(entry_id=item['entry_id'],\n"
            "                                   categories=[...])\n"
