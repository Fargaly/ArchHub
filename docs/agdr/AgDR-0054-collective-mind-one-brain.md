...
### The DRIVE — completion-enforcing Stop hook (founder request 2026-06-13: *"can I have a system configuration to give you a sense of responsibility?"*)
The agent has **no intrinsic drive-to-completion** — it does not persist between turns and feels no pressure from the undone (founder named this exactly). The fix is not a value to instill but a **config that refuses the exit until "done" is true** — the *actuator* of S2/S4. A **Stop hook** (`tools/completion_gate.py`, wired in `~/.claude/settings.json` → `Stop`, before the existing `skill_mint`) fires the instant the agent tries to end a turn: it reads the active-work ledger's done-gates (S1) and, if any are red and machine-resolvable within an iteration/budget cap, returns `{"decision":"block","reason":"NOT DONE: <red leaves>; continue"}` — handing the agent's own "done" back as "keep going." That refusal is the externalized drive. **Two exits only:** (a) all gates green → stop allowed; (b) a gate is `needs_root` (not machine-finishable) OR the cap is hit → **escalate to the founder** with an honest "stuck — need you." Never a silent quit, never a fake-done, never an infinite grind. **Un-bypassable requirement (the bypass already observed):** the authoritative "done" cannot live in a hook the agent can edit or skip — it is integrity-checked and ultimately held by the **server-side verify (slice-4)**; a local hook the agent can disable is exactly how it routes around the rule. This is the in-process expression of ROMA `sweep().dry` + the never-reward-short diligence gate — part of **S2 (load+obey) + S4 (done-rule)**, built at the **FRONT of the core** (it is the drive the founder is missing), not later.

24. **The drive fires**: an agent that tries to end a turn with a red/unmet done-gate is BLOCKED by the Stop hook and re-entered with the unfinished list; a `needs_root`/cap-exceeded gate escalates to the founder (honest stop) instead of a silent quit or a fake done; and the gate cannot be satisfied by a hook the agent can disable (server-authoritative done).

### Universal enforcement — no "later", no partial, ALL agents (founder request 2026-06-13)
The drive must also (a) stop deferral at PLAN time, (b) make partial unacceptable, (c) bind EVERY agent — not just hook-bearing Claude Code. The honest constraint: "all agents" cannot be a conscience handed to each one (any agent can lack or skip a local hook — the bypass already observed). It must be enforced at the **shared choke points every agent must cross, where none can disable it.**

- **No "later" as a legal state (plan-time).** The requirement-tree/ledger has THREE leaf states only — `open` (must be worked), `green` (done), `needs_root` (founder). There is **no** "later"/"deferred" status to park work in. A not-now item is legal ONLY with a machine-checkable justified-hold tag: `depends-on:<leaf-id>` or `safety-gated:<reason>` (what S5 carries). Bare deferral — "later / phase-2 / follow-up / partial / for-hardening / nice-to-have / TODO(later)" — is REJECTED at intake by a **plan-lint gate** (pre-commit + CI scan of `docs/ROADMAP.md` + `docs/agdr/*` + task intake). Legitimate sequencing survives because it carries a reason; cowardly deferral cannot exist on paper.
- **Partial is not done (derive, don't declare).** A parent is `green` IFF every child is `green` (ROMA `sweep`); a partial parent stays red by construction. Per-task definition-of-done = all acceptance gates green; **CI blocks merge while any gate is red**. A partial build cannot land.
- **All agents — at the shared chokepoints, not per-agent.** Local hooks (Stop + plan-lint) are the fast early catch for agents that support them (Claude Code) but are **not authoritative**. Authority lives where every agent must pass:
  - **Brain (`:8473`)** — BRAIN-FIRST makes connection compulsory; the brain owns the tree and **adjudicates "done" server-side** (`sweep().dry` + slice-4 re-execution), not the agent; it refuses a plan carrying un-tagged deferral and refuses to mark a red task done.
  - **Git / CI (server-side, un-bypassable)** — pre-commit + CI rejects deferral markers in touched plans/code, blocks merge on any red/partial gate, and blocks `--no-verify`/force-push (#P0). Catches **Codex · Gemini · composer · fleet · humans** — any committer, any tool, because it runs on the repo, not in the agent.
  - **Honest limit:** an agent that neither commits nor touches the brain is outside the net by construction — but then nothing it does SHIPS (git/CI) or counts as shared truth (brain), so the net holds where it matters.

25. **No-later gate**: a roadmap/AgDR/task item with bare deferral language and no `depends-on:`/`safety-gated:` tag is REJECTED by plan-lint (pre-commit + CI); a properly-tagged hold passes.
26. **Partial-cannot-land**: a parent with any red child is not `green` (sweep) AND CI blocks its merge; a deliberately-partial PR fails.
27. **All-agents coverage**: the same done-adjudication + no-later + no-partial gates fire for a task driven by Claude Code, by Codex, and by a fleet/composer agent — enforced brain-side + CI-side, not only on the hook-bearing client.
